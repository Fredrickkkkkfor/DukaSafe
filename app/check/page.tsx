...
import { ActionPanel, Badge, Card, EmptyState, LinkButton, TrustBadge, formatStatus } from "@/components/ui";
      ...
      <PageShell className="space-y-6">
        <section className="grid gap-5 lg:grid-cols-[1fr_0.55fr]">
          <Card>
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-sage">
              {["tiktok.com/@aisha.styles", "instagram.com/aishastyles.nrb", "+254712345678", "dukasafe.co.ke/s/aisha-styles-nairobi"].map((example) => <a key={example} href={`/check?q=${encodeURIComponent(example)}`} className="rounded-full bg-sand px-3 py-2 hover:bg-amber/20">{example}</a>)}
            </div>
          </Card>
          <Card className="flex flex-col justify-center border border-amber/30 bg-amber/15 text-forest">
            <AlertTriangle className="h-10 w-10 text-[#9a6428]" />
            <h2 className="mt-4 text-2xl font-black">Never send money directly to a number.</h2>
            <p className="mt-3 text-sm leading-6 text-charcoal/75">If the seller cannot provide a verified checkout link, use DukaSafe to record terms, proof, and dispute evidence.</p>
            <div className="mt-5 grid gap-2 text-sm font-bold">
              <LinkButton href="/check" variant="secondary">Verify another seller</LinkButton>
              <LinkButton href="/report-concern" variant="ghost">Report concern</LinkButton>
            </div>
          </Card>
        </section>
        ...
        <section className="grid gap-4">
          {query.q && !sellers.length && (
            <EmptyState title="Seller not verified on DukaSafe yet" body="This does not automatically mean they are unsafe. It means DukaSafe has not verified this seller yet. Ask them for a DukaSafe protected checkout link before paying." action={<div className="flex flex-col justify-center gap-2 sm:flex-row"><LinkButton href="/check" variant="secondary">Verify another seller</LinkButton><LinkButton href={`/report-concern?seller=${encodeURIComponent(query.q)}`} variant="secondary">Report concern</LinkButton><LinkButton href="/protection-charter" variant="secondary">Read buyer safety rules</LinkButton></div>} />
          )}
          {sellers.map((seller) => (
            <Card key={seller.id} className="grid gap-5 md:grid-cols-[1fr_14rem]">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge tone={seller.verified ? "green" : seller.seller_status === "suspended" ? "red" : "gold"}>{seller.verified ? "Verified seller found" : seller.seller_status === "suspended" ? "Seller suspended" : "Under review"}</Badge>
                </div>
